...
pub struct RunAsPpl;
impl RunAsPpl { pub fn new() -> Self { Self } }
impl Default for RunAsPpl { fn default() -> Self { Self::new() } }

#[async_trait]
impl NxcModule for RunAsPpl {
    fn name(&self) -> &'static str { "runasppl" }
    fn description(&self) -> &'static str { "Check if LSASS is running as Protected Process Light (RunAsPPL)" }
    ...
    async fn run(&self, session: &mut dyn NxcSession, _opts: &ModuleOptions) -> Result<ModuleResult> {
        ...
        Ok(ModuleResult { success: true, output, data: json!({"runasppl_check": "requires_admin"}), credentials: vec![] })
    }
}
