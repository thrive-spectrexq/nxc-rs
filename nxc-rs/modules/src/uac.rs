...
pub struct Uac;
impl Uac { pub fn new() -> Self { Self } }
impl Default for Uac { fn default() -> Self { Self::new() } }

#[async_trait]
impl NxcModule for Uac {
    fn name(&self) -> &'static str { "uac" }
    fn description(&self) -> &'static str { "Check remote UAC settings (LocalAccountTokenFilterPolicy, FilterAdministratorToken)" }
    ...
    async fn run(&self, session: &mut dyn NxcSession, _opts: &ModuleOptions) -> Result<ModuleResult> {
        ...
        output.push_str("  [*] HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\System\n");
        ...
        Ok(ModuleResult { success: true, output, data: json!({"uac_check": true}), credentials: vec![] })
    }
}
