...
impl GetModule {
    async fn run_smb(&self, session: &mut dyn NxcSession, path_full: &str) -> Result<ModuleResult> {
        ...
        let (share, path) = path_full.split_once('\\').ok_or_else(|| anyhow::anyhow!("PATH must be in format SHARE\\path"))?;
        ...
        match protocol.download_file(smb_sess, share, path).await {
            Ok(data) => {
                ...
                Ok(ModuleResult {
                    ...
                    output: format!("[+] Successfully downloaded {} to {}", path_full, local_path),
