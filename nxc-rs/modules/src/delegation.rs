...
impl Delegation { pub fn new() -> Self { Self } }
impl Default for Delegation { fn default() -> Self { Self::new() } }

#[async_trait]
impl NxcModule for Delegation {
    fn name(&self) -> &'static str { "delegation" }
    fn description(&self) -> &'static str { "Find unconstrained, constrained, and RBCD delegation configurations" }
    ...
    async fn run(&self, session: &mut dyn NxcSession, _opts: &ModuleOptions) -> Result<ModuleResult> {
        ...
        let uc_entries = proto.search(ldap_sess, &base_dn, ldap3::Scope::Subtree, uc_filter, vec!["sAMAccountName", "distinguishedName"]).await?;
        ...
        let cd_entries = proto.search(ldap_sess, &base_dn, ldap3::Scope::Subtree, cd_filter, vec!["sAMAccountName", "msDS-AllowedToDelegateTo"]).await?;
        output.push_str(&format!("\n  [+] Constrained Delegation ({}):\n", cd_entries.len()));
        for e in &cd_entries {
            let name = e.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned().unwrap_or_default();
        ...
        let rbcd_entries = proto.search(ldap_sess, &base_dn, ldap3::Scope::Subtree, rbcd_filter, vec!["sAMAccountName"]).await?;
        ...
        Ok(ModuleResult { success: total > 0, output, data: json!({"unconstrained": uc_entries.len(), "constrained": cd_entries.len(), "rbcd": rbcd_entries.len()}), credentials: vec![] })
    }
}
