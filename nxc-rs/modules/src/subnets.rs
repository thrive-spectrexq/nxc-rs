...
pub struct Subnets;
impl Subnets { pub fn new() -> Self { Self } }
impl Default for Subnets { fn default() -> Self { Self::new() } }

#[async_trait]
impl NxcModule for Subnets {
    fn name(&self) -> &'static str { "subnets" }
    fn description(&self) -> &'static str { "Enumerate AD Sites and Services subnets" }
    ...
    async fn run(&self, session: &mut dyn NxcSession, _opts: &ModuleOptions) -> Result<ModuleResult> {
        ...
        let entries = proto.search(ldap_sess, &config_dn, ldap3::Scope::Subtree, "(objectClass=subnet)", vec!["cn", "siteObject", "description"]).await?;
        let mut output = format!("AD Subnets ({}):\n", entries.len());
        let mut subnets = Vec::new();
        for e in &entries {
            let name = e.attrs.get("cn").and_then(|v| v.first()).cloned().unwrap_or_default();
            let site = e.attrs.get("siteObject").and_then(|v| v.first()).cloned().unwrap_or_default();
        ...
        Ok(ModuleResult { success: !entries.is_empty(), output, data: json!({"subnets": subnets}), credentials: vec![] })
    }
}
