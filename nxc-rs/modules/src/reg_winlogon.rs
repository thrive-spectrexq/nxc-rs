...
impl RegWinlogon { pub fn new() -> Self { Self } }
impl Default for RegWinlogon { fn default() -> Self { Self::new() } }

#[async_trait]
impl NxcModule for RegWinlogon {
    fn name(&self) -> &'static str { "reg_winlogon" }
    fn description(&self) -> &'static str { "Extract DefaultUserName/DefaultPassword from Winlogon registry" }
    ...
    async fn run(&self, session: &mut dyn NxcSession, _opts: &ModuleOptions) -> Result<ModuleResult> {
        ...
        Ok(ModuleResult { success: true, output, data: json!({"winlogon_check": true}), credentials: vec![] })
    }
}
