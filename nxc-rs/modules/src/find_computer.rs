...
impl NxcModule for FindComputer {
    ...
    fn description(&self) -> &'static str {
        "Find computer objects in AD with optional OS filtering"
    }
    ...
    fn options(&self) -> Vec<ModuleOption> {
        vec![ModuleOption {
            name: "OS".into(),
            description: "Filter by OS (e.g. 'Windows Server 2019')".into(),
            required: false,
            default: None,
        }]
    }
    async fn run(
        ...
        let os_filter =
            opts.get("OS").map(|os| format!("(operatingSystem=*{os}*)")).unwrap_or_default();
        ...
        for e in &entries {
            ...
            let dns =
                e.attrs.get("dNSHostName").and_then(|v| v.first()).cloned().unwrap_or_default();
            output.push_str(&format!("  [+] {name} | {os} | {dns}\n"));
            computers.push(json!({"name": name, "os": os, "dns": dns}));
        }
        Ok(ModuleResult {
            success: true,
            output,
            data: json!({"computers": computers}),
            credentials: vec![],
        })
    }
}
