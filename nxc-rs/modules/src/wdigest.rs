...
impl NxcModule for Wdigest {
    ...
    fn description(&self) -> &'static str {
        "Enable/disable WDigest plaintext credential caching (UseLogonCredential)"
    }
    ...
    async fn run(
        ...
        match action {
            "enable" => output
                .push_str("  [*] Would set UseLogonCredential = 1 (requires admin + reboot)\n"),
            "disable" => output
                .push_str("  [*] Would set UseLogonCredential = 0 (requires admin + reboot)\n"),
