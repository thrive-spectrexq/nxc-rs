...
use nxc_protocols::rpc::{UUID_SAMR, PacketType};
...
impl NxcModule for Secretsdump {
    ...
    async fn run(&self, session: &dyn NxcSession, _opts: &ModuleOptions) -> Result<ModuleResult> {
        ...
        let resp = match protocol.call_rpc(
            smb_session, 
            "samr", 
            PacketType::Bind, 
            1, 
        ...
        if resp.len() >= 3 && resp[2] == 12 {
            ...
            output_lines.push("[*] Dumping local SAM hashes (not really, this is a demonstration)".to_string());
