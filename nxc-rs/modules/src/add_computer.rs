...
impl AddComputer { pub fn new() -> Self { Self } }
impl Default for AddComputer { fn default() -> Self { Self::new() } }

#[async_trait]
impl NxcModule for AddComputer {
    fn name(&self) -> &'static str { "add_computer" }
    fn description(&self) -> &'static str { "Add a machine account to the domain using MachineAccountQuota" }
    fn supported_protocols(&self) -> &[&str] { &["ldap"] }
    fn options(&self) -> Vec<ModuleOption> {
        vec![
            ModuleOption { name: "NAME".into(), description: "Computer account name to create".into(), required: true, default: None },
            ModuleOption { name: "PASSWORD".into(), description: "Password for the new computer account".into(), required: true, default: None },
        ]
    }
    async fn run(&self, session: &mut dyn NxcSession, opts: &ModuleOptions) -> Result<ModuleResult> {
        ...
        let mut output = format!("Add Computer Account:\n");
        output.push_str(&format!("  [*] Computer name: {}$\n", name));
        output.push_str(&format!("  [*] Base DN: {}\n", base_dn));
        output.push_str(&format!("  [*] Password: {}\n", "*".repeat(password.len())));
        output.push_str("  [*] Will use ms-DS-MachineAccountQuota for creation\n");
        Ok(ModuleResult { success: true, output, data: json!({"computer": name, "base_dn": base_dn}), credentials: vec![] })
    }
}
