...
impl NxcModule for GroupMem {
    ...
    fn description(&self) -> &'static str {
        "Enumerate members of an AD group (recursive)"
    }
    ...
    fn options(&self) -> Vec<ModuleOption> {
        vec![ModuleOption {
            name: "GROUP".into(),
            description: "Group name to enumerate (default: Domain Admins)".into(),
            required: false,
            default: Some("Domain Admins".into()),
        }]
    }
    async fn run(
        &self,
        ...
        let entries = proto
            ...
            .await?;
        let mut output = format!("Group Membership for '{group}':\n");
        ...
        if members.is_empty() {
            output.push_str("  [-] No members found or group not found\n");
        }
        Ok(ModuleResult {
            success: !members.is_empty(),
            output,
            data: json!({"group": group, "members": members}),
            credentials: vec![],
        })
    }
}
