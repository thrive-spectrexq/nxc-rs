...
impl GetInfoUsers { pub fn new() -> Self { Self } }
impl Default for GetInfoUsers { fn default() -> Self { Self::new() } }

#[async_trait]
impl NxcModule for GetInfoUsers {
    fn name(&self) -> &'static str { "get_info_users" }
    fn description(&self) -> &'static str { "Dump detailed user attributes (lastLogon, pwdLastSet, badPwdCount, etc.)" }
    ...
    async fn run(&self, session: &mut dyn NxcSession, _opts: &ModuleOptions) -> Result<ModuleResult> {
        ...
        let entries = proto.search(ldap_sess, &base_dn, ldap3::Scope::Subtree,
            ...
            vec!["sAMAccountName", "lastLogon", "pwdLastSet", "badPwdCount", "logonCount", "userAccountControl", "adminCount"]).await?;
        ...
        for e in &entries {
            ...
            let admin_count = e.attrs.get("adminCount").and_then(|v| v.first()).cloned().unwrap_or_default();
            let bad_pwd = e.attrs.get("badPwdCount").and_then(|v| v.first()).cloned().unwrap_or_default();
            if admin_count == "1" { output.push_str(&format!("  [!] {} (adminCount=1, badPwd={})\n", name, bad_pwd)); }
            results.push(json!({"user": name, "adminCount": admin_count, "badPwdCount": bad_pwd}));
        }
        Ok(ModuleResult { success: true, output, data: json!({"users": results}), credentials: vec![] })
    }
}
