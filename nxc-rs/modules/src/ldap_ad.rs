...
impl NxcModule for LdapAdModule {
    ...
    async fn run(&self, session: &mut dyn NxcSession, opts: &ModuleOptions) -> Result<ModuleResult> {
        ...
        if enum_type == "trusts" || enum_type == "all" {
            ...
            output.push_str(&format!("\n[+] Domain Trusts ({}):\n", trusts.len()));
            for trust in &trusts {
                output.push_str(&format!("  - {} (Direction: {}, Type: {})\n", 
...
impl LdapAdModule {
    async fn enum_gpos(&self, proto: &LdapProtocol, session: &LdapSession) -> Result<Vec<serde_json::Value>> {
        let base_dn = proto.get_base_dn(session).await?;
        let filter = "(objectCategory=groupPolicyContainer)";
        let entries = proto.search(session, &base_dn, ldap3::Scope::Subtree, filter, vec!["displayName", "gPCFileSysPath"]).await?;
        
        let mut results = Vec::new();
        for entry in entries {
            let name = entry.attrs.get("displayName").and_then(|v| v.first()).cloned().unwrap_or_default();
            let path = entry.attrs.get("gPCFileSysPath").and_then(|v| v.first()).cloned().unwrap_or_default();
            results.push(json!({"name": name, "path": path}));
        }
        Ok(results)
    }

    async fn enum_trusts(&self, proto: &LdapProtocol, session: &LdapSession) -> Result<Vec<serde_json::Value>> {
        let base_dn = proto.get_base_dn(session).await?;
        let filter = "(objectClass=trustedDomain)";
        let entries = proto.search(session, &base_dn, ldap3::Scope::Subtree, filter, vec!["trustPartner", "trustDirection", "trustType"]).await?;
        
        let mut results = Vec::new();
        for entry in entries {
            let partner = entry.attrs.get("trustPartner").and_then(|v| v.first()).cloned().unwrap_or_default();
            let direction = entry.attrs.get("trustDirection").and_then(|v| v.first()).cloned().unwrap_or_default();
            ...
            let t_type = entry.attrs.get("trustType").and_then(|v| v.first()).cloned().unwrap_or_default();
            results.push(json!({"partner": partner, "direction": direction_str, "type": t_type}));
        }
        Ok(results)
    }

    async fn enum_delegation(&self, proto: &LdapProtocol, session: &LdapSession) -> Result<Vec<serde_json::Value>> {
        let base_dn = proto.get_base_dn(session).await?;
        // Unconstrained: userAccountControl & 0x80000
        // Constrained: msDS-AllowedToDelegateTo exists
        let filter = "(|(userAccountControl:1.2.840.113556.1.4.803:=524288)(msDS-AllowedToDelegateTo=*))";
        let entries = proto.search(session, &base_dn, ldap3::Scope::Subtree, filter, vec!["sAMAccountName", "userAccountControl", "msDS-AllowedToDelegateTo"]).await?;
        
        let mut results = Vec::new();
        for entry in entries {
            let name = entry.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned().unwrap_or_default();
            let uac = entry.attrs.get("userAccountControl").and_then(|v| v.first()).and_then(|v| v.parse::<u32>().ok()).unwrap_or(0);
            ...
            if entry.attrs.contains_key("msDS-AllowedToDelegateTo") {
                if !d_type.is_empty() { d_type.push_str(", "); }
