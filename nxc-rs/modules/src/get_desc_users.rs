...
impl NxcModule for GetDescUsers {
    ...
    fn description(&self) -> &'static str {
        "Dump user description attributes (may contain credentials)"
    }
    ...
    async fn run(
        ...
        let entries = proto
            ...
            .await?;
        let mut output =
            format!("User Descriptions ({} users with descriptions):\n", entries.len());
        let mut results = Vec::new();
        for e in &entries {
            let name =
                e.attrs.get("sAMAccountName").and_then(|v| v.first()).cloned().unwrap_or_default();
            let desc =
                e.attrs.get("description").and_then(|v| v.first()).cloned().unwrap_or_default();
