...
impl NxcModule for AdcsModule {
    ...
    async fn run(&self, session: &mut dyn NxcSession, _opts: &ModuleOptions) -> Result<ModuleResult> {
        ...
        if let Some(ldap_sess) = session.as_any().downcast_ref::<nxc_protocols::ldap::LdapSession>() {
            ...
            let ca_entries = protocol.search(ldap_sess, &ca_dn, ldap3::Scope::OneLevel, "(objectClass=certificationAuthority)", vec!["cn"]).await?;
            ...
            let enroll_dn = format!("CN=Enrollment Services,CN=Public Key Services,CN=Services,{}", config_dn);
            let enroll_entries = protocol.search(ldap_sess, &enroll_dn, ldap3::Scope::OneLevel, "(objectClass=pkiEnrollmentService)", vec!["cn", "dNSHostName", "cACertificate"]).await?;
            
            output.push_str("\n[+] Enrollment Services (Active CAs):\n");
            for entry in enroll_entries {
                let cn = entry.attrs.get("cn").and_then(|v| v.first()).cloned().unwrap_or_default();
                let host = entry.attrs.get("dNSHostName").and_then(|v| v.first()).cloned().unwrap_or_default();
            ...
            let template_entries = protocol.search(ldap_sess, &template_dn, ldap3::Scope::OneLevel, "(objectClass=pkicertificateTemplate)", vec!["cn", "displayName", "msPKI-Certificate-Name-Flag", "msPKI-Enrollment-Flag", "msPKI-RA-Signature", "pKIExtendedKeyUsage"]).await?;
            ...
            for entry in template_entries {
                ...
                let name_flags = entry.attrs.get("msPKI-Certificate-Name-Flag").and_then(|v| v.first()).and_then(|s| s.parse::<u32>().ok()).unwrap_or(0);
                let enrollment_flags = entry.attrs.get("msPKI-Enrollment-Flag").and_then(|v| v.first()).and_then(|s| s.parse::<u32>().ok()).unwrap_or(0);
                let _ra_signature = entry.attrs.get("msPKI-RA-Signature").and_then(|v| v.first()).and_then(|s| s.parse::<u32>().ok()).unwrap_or(0);
            ...
            return Err(anyhow::anyhow!("Module adcs only supports LDAP protocol session downcasting."));
        }
