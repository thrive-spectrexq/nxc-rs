...
impl NxcProtocol for WmiProtocol {
    ...
    async fn connect(&self, target: &str, port: u16) -> Result<Box<dyn NxcSession>> {
        ...
        let bind_req: [u8; 72] = [
            ...
            0x02, 0x00, 0x02, 0x00, 0x08, 0x83, 0xaf, 0xe1, 
    ...
    async fn authenticate(
        ...
        Ok(AuthResult::failure("WMI DCOM/IWbemLevel1Login explicit NTLM logic pending implementation", None))
    }

    async fn execute(&self, _session: &dyn NxcSession, _cmd: &str) -> Result<CommandOutput> {
        Err(anyhow!("WMI explicit command execution (`Win32_Process.Create`) not yet ported."))
    }
}
