...
impl NxcSession for IpmiSession {
    fn protocol(&self) -> &'static str { "ipmi" }
...
impl NxcProtocol for IpmiProtocol {
    ...
    async fn connect(&self, target: &str, port: u16, _proxy: Option<&str>) -> Result<Box<dyn NxcSession>> {
        ...
        let bmc_info = match timeout(Duration::from_secs(3), socket.recv_from(&mut buf)).await {
            Ok(Ok((n, _))) => {
                ...
                Some(format!("BMC responded with {} bytes", n))
            }
            _ => {
    ...
    async fn authenticate(&self, session: &mut dyn NxcSession, creds: &Credentials) -> Result<AuthResult> {
        let ipmi_sess = session.as_any_mut().downcast_mut::<IpmiSession>()
...
impl IpmiProtocol {
    ...
    pub async fn dump_rakp_hash(&self, session: &IpmiSession, username: &str) -> Result<Option<String>> {
