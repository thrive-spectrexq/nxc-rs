...
impl NxcProtocol for NfsProtocol {
    ...
    async fn read_file(
        ...
        Err(anyhow!(
            "Full NFS v3 READ requires iterative LOOKUP which is pending RPC structural port."
        ))
    }
    ...
    async fn write_file(
        ...
        Err(anyhow!(
            "Full NFS v3 WRITE requires iterative LOOKUP which is pending RPC structural port."
        ))
    }
}
...
impl NfsProtocol {
    ...
    pub async fn list_exports(&self, target: &str) -> Result<Vec<String>> {
        ...
        while pos + 4 <= body.len() && body[pos + 3] == 1 {
            ...
            let len = u32::from_be_bytes(
                body[pos..pos + 4].try_into().unwrap_or_else(|_| panic!("Invalid bytes length")),
            ) as usize;
            pos += 4;
            ...
            while pos + 4 <= body.len() && body[pos + 3] == 1 {
                ...
                let h_len = u32::from_be_bytes(
                    body[pos..pos + 4]
                        .try_into()
                        .unwrap_or_else(|_| panic!("Invalid bytes length")),
                ) as usize;
                pos += 4;
                pos += (h_len + 3) & !3;
                if pos + 4 > body.len() {
    ...
    async fn get_rpc_port(&self, target: &str, program: u32, version: u32) -> Result<u16> {
        ...
        let port = u32::from_be_bytes(
            resp[24..28].try_into().unwrap_or_else(|_| panic!("Invalid bytes length")),
        ) as u16;
        Ok(port)
    }
}
