...
impl RegistrySecrets {
    ...
    pub fn get_boot_key(system_hive_data: &[u8]) -> Result<[u8; 16]> {
        let hive = Hive::new(system_hive_data).map_err(|e| anyhow!("Failed to parse SYSTEM hive: {}", e))?;
        let root = hive.root_key_node().map_err(|e| anyhow!("No root key node in SYSTEM hive: {}", e))?;
        ...
        if decoded.len() < 16 { return Err(anyhow!("Invalid boot key length")); }
    ...
    pub fn get_sam_hashes(sam_data: &[u8], boot_key: &[u8; 16]) -> Result<Vec<(String, String)>> {
        ...
        let root = hive.root_key_node().map_err(|e| anyhow!("No root key node in SAM hive: {}", e))?;
        
        let users_node = root.subpath("SAM\\Domains\\Account\\Users")
            .ok_or_else(|| anyhow!("Users key not found"))?
            .map_err(|e| anyhow!("Error accessing Users key: {}", e))?;
        
        let names_node = users_node.subpath("Names")
        ...
        if let Some(subkeys_res) = names_node.subkeys() {
            ...
            for subkey_res in subkeys.into_iter() {
                let node: KeyNode<'_, _> = subkey_res.map_err(|e| anyhow!("Error accessing subkey: {}", e))?;
                let rid_hex = node.class_name()
                    .transpose()
                    .map_err(|e| anyhow!("Error getting classname: {}", e))?
                    .map(|s| s.to_string())
                    .unwrap_or_default();
                let rid = if rid_hex.len() >= 8 { u32::from_str_radix(&rid_hex[..8], 16).unwrap_or(0) } else { 0 };
                rid_to_name.insert(rid, node.name().map_err(|e| anyhow!("Error getting node name: {}", e))?.to_string());
            }
        }
        ...
        if let Some(subkeys_res) = users_node.subkeys() {
            ...
            for subkey_res in subkeys.into_iter() {
                let user_node: KeyNode<'_, _> = subkey_res.map_err(|e| anyhow!("Error accessing subkey: {}", e))?;
                let rid_str = user_node.name().map_err(|e| anyhow!("Error getting node name: {}", e))?.to_string();
                if rid_str == "Names" { continue; }
                
                let rid = u32::from_str_radix(&rid_str, 16).unwrap_or(0);
                let username = rid_to_name.get(&rid).cloned().unwrap_or_else(|| format!("User_{}", rid));
                
                if let Some(v_val_res) = user_node.value("V") {
                    let v_val: KeyValue<'_, _> = v_val_res.map_err(|e| anyhow!("Error getting V value: {}", e))?;
    ...
    fn decrypt_sam_hash(v_data: &[u8], boot_key: &[u8; 16], rid: u32, hash_type: &str) -> Result<String> {
        if v_data.len() < 160 { return Err(anyhow!("V structure too short")); }
        
        let off = if hash_type == "NT" { 164 } else { 152 }; // Simplified offsets
        if v_data.len() < off + 16 { return Err(anyhow!("Hash offset beyond V length")); }
    ...
    fn get_classname_by_path(root: &KeyNode<'_, &[u8]>, path: &str) -> Result<String> {
        let node: KeyNode<'_, &[u8]> = root.subpath(path)
