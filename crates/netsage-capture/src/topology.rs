...
impl TopologyGraph {
    ...
    pub fn add_node(&mut self, id: String, label: String, node_type: String) {
        self.nodes.insert(id.clone(), Node { id, label, node_type });
    }

    pub fn add_edge(&mut self, from: String, to: String) {
        if from != to && !self.edges.iter().any(|e| (e.from == from && e.to == to) || (e.from == to && e.to == from)) {
            self.edges.push(Edge { from, to, metadata: HashMap::new() });
        }
    }
    ...
    pub fn to_ascii(&self) -> String {
        ...
        for (id, node) in &self.nodes {
            ...
            output.push_str(&format!("  [{}] {}\n", node.node_type.to_uppercase(), node.label));
            seen.insert(id.clone());

            for edge in &self.edges {
                if &edge.from == id {
                    let to_node = self.nodes.get(&edge.to).map(|n| format!("[{}] {}", n.node_type.to_uppercase(), n.label)).unwrap_or_else(|| edge.to.clone());
                    
                    let mut meta_str = String::new();
                    if !edge.metadata.is_empty() {
                        meta_str = format!(" ({})", edge.metadata.iter().map(|(k, v)| format!("{}: {}", k, v)).collect::<Vec<_>>().join(", "));
                    }
